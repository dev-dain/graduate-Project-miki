const createModal = (modalContainer: HTMLDivElement, state: string, msg?: string): HTMLDivElement => {
  const modalBG: HTMLDivElement = document.createElement('div');
  modalBG.classList.add('modal-background');

  const modalBox: HTMLDivElement = document.createElement('div');
  modalBox.classList.add('modal-box');

  const modalCloseBtn: HTMLSpanElement = document.createElement('span');
